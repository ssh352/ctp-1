...
namespace CalmBeltFund.Trading.CTP
{
  public partial class CTPTrader : CTPFutureClient
  {
    ...
    public void Connect(string[] frontAddress, string brokerID, string userID, string password,bool restart = true)
    {
      ...
      {
        //创建
          _instance = (IntPtr)Process(Marshal.GetFunctionPointerForDelegate(this.callback), (int)CTPRequestAction.TraderApiCreate, 0, new StringBuilder(connTempFile));
        ...
        //订阅私有流
        Process(this._instance, (int)CTPRequestAction.TraderApiSubscribePrivateTopic, (int)CTPResumeType.TERT_QUICK, null);
        //订阅公共流
        Process(this._instance, (int)CTPRequestAction.TraderApiSubscribePublicTopic, (int)CTPResumeType.TERT_RESUME, null);
        //注册前置机地址
        foreach (string front in frontAddress)
        {
          ...
          {//不是以tcp开头的就认为是域名连接
              address = "tcp://" + address;
              //Process(this._instance, (int)CTPRequestAction.TraderApiRegisterNameServer, 0, new StringBuilder(address));
          }
          else
          {
    ...
    public void UserLogin(string userId, string pwd)
    {
        this.InvestorID = userId;
        this.Password = pwd;
    ...
    public void QueryTransferSerial()
    {
      ...
      /// 断开重连时间间隔（秒）
      /// </summary>
      private int reconnectStepTime = 3;
      /// <summary>
      /// 最大重连次数
      /// </summary>
                ...
                case CTPResponseType.UserLoginResponse:
                    {
                        ...
                        {                            
                            ...
                            //上海
                            if (!TimeSpan.TryParse(userLogin.SHFETime, out this.loginTimeSHFE))
                            {
                                this.loginTimeSHFE = DateTime.Now.TimeOfDay;
                            }
